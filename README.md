...
FreeMail exposes a stateless MCP server at **`POST {api}/mcp`**, authenticated with the API key you created in the web app. It always offers **`send_email`**; when **inbound is enabled** it also offers the read tools **`list_emails`**, **`get_email`**, and **`get_email_attachment_url`** so agents can read received mail. (Inbound email is untrusted external content — the read tools flag it as data, not instructions.)
