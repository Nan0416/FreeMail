...
import type { EmailsRepo, InboundEmailRecord } from '../data/emails-repo.js';
import type { InboundObjectStore, ObjectHead } from '../data/inbound-object-store.js';
import { MAX_ATTACHMENTS } from './limits.js';
import { ATTACHMENTS_PREFIX, InboundProcessor } from './processor.js';
...
class FakeStore implements InboundObjectStore {
  ...
  putAttachment(key: string): Promise<void> {
    if (this.putShouldThrow) return Promise.reject(new Error('s3 put failed'));
...
class FakeRepo implements EmailsRepo {
  ...
  putInbound(record: InboundEmailRecord): Promise<boolean> {
    if (this.existingIds.has(record.id)) return Promise.resolve(false);
