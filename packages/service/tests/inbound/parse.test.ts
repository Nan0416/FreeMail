...
import type { InboundAttachmentDescriptor } from '../../src/data/emails-repo.js';
import { parseInbound, type AttachmentSink, type ParseLimits } from '../../src/inbound/parse.js';
...
describe('parseInbound — limits', () => {
  ...
  it('limit_exceeded on more than maxChildNodes REAL nodes (non-attachment text parts)', async () => {
    ...
    for (let i = 0; i < 6; i++) {
      many.push('--B', 'Content-Type: text/plain', '', `part ${i}`);
    }
    many.push('--B--', '');
