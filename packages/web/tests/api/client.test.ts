...
import { ApiError, FreeMailClient } from '../../src/api/client.js';
...
describe('FreeMailClient (cookie auth)', () => {
  ...
  it('fires onAuthLost when the refresh itself fails (non-2xx)', async () => {
    ...
    const fetchMock = vi.fn<typeof fetch>(async (url) => {
      const path = new URL(String(url)).pathname;
      if (path === '/me') {
        return json(403, { error: 'invalid_token', message: 'expired' });
      }
      if (path === '/auth/refresh') {
        return json(401, { error: 'invalid_token', message: 'gone' });
      }
      throw new Error(`unexpected ${path}`);
    });
  ...
  it('fires onAuthLost when the refresh request THROWS (network error)', async () => {
    ...
    const fetchMock = vi.fn<typeof fetch>(async (url) => {
      const path = new URL(String(url)).pathname;
      if (path === '/me') {
        return json(401, { error: 'invalid_token', message: 'expired' });
      }
      if (path === '/auth/refresh') {
